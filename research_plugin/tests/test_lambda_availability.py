...
from backend.execution.errors import BackendValidationError
...
class LambdaAvailabilityTest(unittest.TestCase):
    ...
    def test_lambda_backend_launches_vm_with_agent_tooling_bootstrap(self) -> None:
        ...
        self.assertEqual(provisioned.workdir, "/workspace/repo")
    ...
    def test_lambda_sync_is_explicitly_not_implemented_yet(self) -> None:
        backend = LambdaLabsSandboxBackend(
            ...
            client=FakeLambdaSandboxClient(),  # type: ignore[arg-type]
        )

        with self.assertRaises(Exception) as ctx:
            backend.sync_sandbox_files(
                project_id="proj1",
                sandbox_id="inst_1",
    ...
    def test_lambda_user_data_contains_remote_workdir_and_data_dir(self) -> None:
        ...
        self.assertIn("RP_WORKDIR=/workspace/repo", user_data)
        self.assertIn("RP_SANDBOX_DATA_DIR=/workspace/sandbox_data", user_data)
