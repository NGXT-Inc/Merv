...
def register_feed_routes(http: Any, *, app_for: Callable[[str], Any]) -> None:
    ...
    def feed(
        ...
        result = app_for(project_id).feed.list_posts(
    ...
    def feed_image(project_id: str, post_id: str) -> Response:
        content, content_type = app_for(project_id).feed.get_image(
            project_id=project_id, post_id=post_id
        )
        return Response(content=content, media_type=content_type)

    @http.get("/api/projects/{project_id}/feed/{post_id}/link-image")
    def feed_link_image(project_id: str, post_id: str) -> Response:
        content, content_type = app_for(project_id).feed.get_link_image(
            project_id=project_id, post_id=post_id
        )
        return Response(content=content, media_type=content_type)

    @http.post("/api/projects/{project_id}/feed/track")
    def feed_track(project_id: str, body: Any = Body(default=None)) -> dict[str, Any]:
        ...
        app_for(project_id).activity.emit(
