    ...
    def recent(
        ...
        *,
        ...
        if source is not None:
            scanned = [event for event in scanned if effective_source(event=event) == source]
    ...
    def record(
        ...
        *,
        ...
        row = {
            ...
            "sent_chars": payload_chars(value=arguments),
            "received_chars": len(error or "") if status == "error" else payload_chars(value=result),
            "error_code": error_code,
        ...
        with self._lock:
            for row in self._calls:
                if int(row["id"]) != int(call_id):
                    continue
                if project_ids is not None and str(row.get("project_id") or "") not in allowed:
                    return None
                return dict(row)
        return None
