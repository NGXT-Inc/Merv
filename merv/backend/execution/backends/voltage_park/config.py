...
class VoltageParkCloudConfig:
    ...
    def from_env(cls) -> "VoltageParkCloudConfig":
        token = (
            os.environ.get("RESEARCH_PLUGIN_VOLTAGE_PARK_TOKEN")
            or os.environ.get("VOLTAGE_PARK_TOKEN")
            or ""
        ).strip()
        if not token:
            raise BackendValidationError(
                "Voltage Park API token is required; set "
                "RESEARCH_PLUGIN_VOLTAGE_PARK_TOKEN or VOLTAGE_PARK_TOKEN"
            )
        base_url = (
            os.environ.get("RESEARCH_PLUGIN_VOLTAGE_PARK_API_BASE") or DEFAULT_BASE_URL
        ).strip()
        if not base_url.startswith(("http://", "https://")):
            raise BackendValidationError(
                "RESEARCH_PLUGIN_VOLTAGE_PARK_API_BASE must be an HTTP URL"
            )
...
class VoltageParkSandboxConfig:
    ...
    def from_env(cls) -> "VoltageParkSandboxConfig":
        return cls(
            ...
            ssh_user=(
                os.environ.get("RESEARCH_PLUGIN_VOLTAGE_PARK_SSH_USER") or DEFAULT_SSH_USER
            ).strip()
            or DEFAULT_SSH_USER,
        )
