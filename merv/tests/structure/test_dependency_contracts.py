...
class DependencyContractTest(unittest.TestCase):
    def test_consolidated_artifacts_api_is_exactly_nine_typed_operations(self) -> None:
        """Freeze the small typed API shared by Research, Application, and Surface."""
        from merv.brain.artifacts import Artifacts
        from merv.brain.artifacts.artifacts import Artifacts as ArtifactsImplementation

        self.assertIs(Artifacts, ArtifactsImplementation)

        expected = {
            "submit": (
                "(self, *, target: 'ArtifactTarget', role: 'str', path: 'str', "
                "lens_id: 'str' = '', title: 'str' = '') -> 'PendingUpload'"
            ),
            "upload_cap": ("(self, *, token: 'str', kind: 'UploadKind') -> 'int'"),
            "complete_upload": (
            ...
            "get": (
                ...
                "include: 'ReadMode' = 'metadata') -> 'tuple[Artifact, ...]'"
            ),
            "scan": (
                "(self, *, project_id: 'str | None' = None, "
                "target_type: 'str' = '', target_ids: 'tuple[str, ...]' = (), "
                "roles: 'tuple[str, ...]' = ()) -> 'tuple[Artifact, ...]'"
            ),
            "figure": (
                "(self, *, artifact_id: 'str', link_path: 'str', "
                "project_id: 'str | None' = None) -> 'bytes | None'"
            ),
            "pin": (
                "(self, *, target: 'ArtifactTarget', role: 'str', path: 'str', "
            ...
            "seal": (
                "(self, *, tx: 'Connection', target: 'ArtifactTarget', "
                "transition: 'str') -> 'None'"
            ),
            "history": (
                "(self, *, tx: 'Connection', target_type: 'str', "
                "target_ids: 'tuple[str, ...]', summarize: 'bool' = False) "
        ...
        self.assertEqual(
            {
                name: str(inspect.signature(getattr(Artifacts, name)))
        ...
        self.assertNotIn(
            ...
            inspect.getsource(Artifacts.seal),
            "seal must participate in Research's caller-owned transaction",
        )
