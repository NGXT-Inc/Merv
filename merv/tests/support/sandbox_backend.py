"""Deterministic Sandbox adapter shared by tests."""
...
from merv.brain.sandbox.adapters.base import price_sort_key
...
from merv.brain.sandbox.remote.transcript_wire import TRANSCRIPT_TAIL_DEFAULT
...
class FakeSandboxBackend(SandboxBackendBase):
    ...
    def read_transcript(
        ...
        *,
        ...
        data = self.transcripts.get(target.experiment_id, "").encode("utf-8")
        total = len(data)
    ...
    def sample_metrics(
        ...
        *,
        ...
        if not self.alive.get(target.sandbox_id):
            return None
        return self.metrics.get(target.sandbox_id)
    ...
    def read_runs(
        ...
        *,
        ...
        if not self.alive.get(target.sandbox_id):
            return None
        return parse_runs_listing(self.run_listings.get(target.sandbox_id, ""))
