...
| Per-provider copies of 404/outage/delete and shared VM lifecycle mechanics | One VM/SSH conformance block plus each provider's billable statuses, routing, availability, authentication, and failure cleanup |
...
Final result: 24 dedicated files and 442 collected cases. This removes 115
...
```text
pytest --collect-only -q merv/tests/sandbox  # 442 collected
pytest -q merv/tests/sandbox                 # 437 passed, 5 skipped
pytest -q merv/tests/application merv/tests/structure merv/tests/surface
# 771 passed
```
