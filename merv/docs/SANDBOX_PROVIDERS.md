...
`RESEARCH_PLUGIN_EXECUTION_BACKEND` (default `lambda_labs`); a fleet is
configured with `RESEARCH_PLUGIN_EXECUTION_BACKENDS` (comma-separated), which
...
- Env: `RESEARCH_PLUGIN_LAMBDA_API_KEY` (or `LAMBDA_LABS_API_KEY` /
  `LAMBDA_API_KEY`); optional `RESEARCH_PLUGIN_LAMBDA_REGION`,
...
- Env: `RESEARCH_PLUGIN_THUNDER_API_KEY` (or `THUNDER_COMPUTE_API_KEY` /
  `TNR_API_TOKEN`).
...
- Env: `RESEARCH_PLUGIN_HYPERSTACK_API_KEY` (or `HYPERSTACK_API_KEY`) and
  `RESEARCH_PLUGIN_HYPERSTACK_ENVIRONMENT`; optional
  `RESEARCH_PLUGIN_HYPERSTACK_IMAGE` (default
  `Ubuntu Server 24.04 LTS (Noble Numbat)`), `RESEARCH_PLUGIN_HYPERSTACK_FLAVOR`.
- Credentials: sign up at <https://console.hyperstack.cloud>, add credit
  ...
  **environment** once in the console (it pins the region) and put its name in
  `RESEARCH_PLUGIN_HYPERSTACK_ENVIRONMENT`.
...
- Env: `RESEARCH_PLUGIN_DIGITALOCEAN_TOKEN` (or `DIGITALOCEAN_TOKEN` /
  `DIGITALOCEAN_ACCESS_TOKEN`); optional `RESEARCH_PLUGIN_DIGITALOCEAN_IMAGE`
  (default `gpu-h100x1-base`, the AI/ML-ready Ubuntu with NVIDIA drivers),
  `RESEARCH_PLUGIN_DIGITALOCEAN_REGION`.
...
- Env: `RESEARCH_PLUGIN_VERDA_CLIENT_ID` + `RESEARCH_PLUGIN_VERDA_CLIENT_SECRET`
  (or `DATACRUNCH_CLIENT_ID`/`DATACRUNCH_CLIENT_SECRET`); optional
  `RESEARCH_PLUGIN_VERDA_IMAGE` (default `ubuntu-24.04`),
  `RESEARCH_PLUGIN_VERDA_LOCATION` (e.g. `FIN-01`).
...
- Quirks: OAuth2 client-credentials (the backend mints and refreshes tokens);
  ...
  (`RESEARCH_PLUGIN_VERDA_API_BASE` overrides).

## Voltage Park (`voltage_park`)

- Env: `RESEARCH_PLUGIN_VOLTAGE_PARK_TOKEN` (or `VOLTAGE_PARK_TOKEN`).
...
- Env: `RESEARCH_PLUGIN_TENSORDOCK_TOKEN` (or `TENSORDOCK_TOKEN`); optional
  `RESEARCH_PLUGIN_TENSORDOCK_IMAGE` (default `ubuntu2404`).
